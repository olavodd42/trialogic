"""Scribe agent for structured clinical data extraction from free text.

This module contains the ScribeAgent class, responsible for invoking an
LLM with structured output constraints to extract vital signs, chief
complaint, and other clinical data from unstructured clinical notes.
"""
...
class ScribeAgent:
    ...
    def __init__(self, model: BaseChatModel, prompt_path: str = "prompts/scribe_prompt.md"):
        ...
        logger.debug("Prompt loaded: %s ...", preview)
    ...
    def process(self, state: AgentState) -> Dict[str, Any]:
        ...
        messages = [
            ...
            HumanMessage(content=f"""
                         Analyze the following clinical text and extract vital signs:
                         \n\n--- BEGIN TEXT ---\n{clinical_text}\n--- END TEXT ---
                    """)
        ]

        if validation_messages and attempts > 0:
            logger.info("Scribe retrying with feedback (%d errors detected).", len(validation_messages))

            # Append error history as user messages (simulating supervisor feedback)
            messages.extend(validation_messages)
            
            # Add a final reinforcement instruction
            reinforcement = (
        ...
        try:
            ...
            logger.info(
                "Extraction successful. HR: %s, BP: %s/%s, Temp: %s, O2Sat: %s",
                domain_vitals.heartrate, domain_vitals.sbp, domain_vitals.dbp,
                domain_vitals.temperature, domain_vitals.o2sat,
            )
            ...
            logger.error("Validation error in Scribe: %s", e)
            return {
            ...
            logger.error("Unexpected error in Scribe agent: %s", e, exc_info=True)
            return {
                "error": str(e),
                "is_success": False
            }
